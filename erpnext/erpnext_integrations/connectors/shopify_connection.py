...
from erpnext.erpnext_integrations.doctype.shopify_settings.shopify_settings import get_header, get_shopify_url
...
def create_sales_order(shopify_order, shopify_settings, company=None):
	...
	if not so:
		items, missing_items = get_order_items(shopify_order.get(
			"line_items"), shopify_settings, getdate(shopify_order.get('created_at')))
